...
```
CODE-- 根据性别分组，统计男性和女性数量（只显示分组数量，不显示哪个是男哪个是女）
...
```
CODE-- 创建用户test，只能在当前主机localhost访问
...
![结构图](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
![层级描述](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
...
InnoDB 逻辑存储结构：
![InnoDB逻辑存储结构](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
...
![二叉树](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)

二叉树的缺点可以用红黑树来解决：
![红黑树](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
红黑树也存在大数据量情况下，层级较深，检索速度慢的问题。

为了解决上述问题，可以使用 B-Tree 结构。
B-Tree (多路平衡查找树) 以一棵最大度数（max-degree，指一个节点的子节点个数）为5（5阶）的 b-tree 为例（每个节点最多存储4个key，5个指针）

![B-Tree结构](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
...
![B+Tree结构图](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
...
![MySQL B+Tree 结构图](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
...
![Hash索引原理图](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
...
![大致原理](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
![演示图](blob:null/03cab5ea-47cc-485d-bb26-9f808d41af90)
...
```
CODEselect * from user where id = 10;
...
```
CODE-- name字段为姓名字段，该字段的值可能会重复，为该字段创建索引
...
```
CODEselect count(distinct email) / count(*) from tb_user;
...
```
CODE# 客户端连接服务端时，加上参数 --local-infile（这一行在bash/cmd界面输入）
...
http://www.zsqblog.top/notes/MySQL笔记/